"""Test configuration for the ElevenLabs integration suite.

`elevenlabs.py` calls a bare `Integration.load()`, which the SDK resolves
relative to its own package location -- correct only when the SDK has been
vendored into `dependencies/` by the Lambda packaging step. Under a normally
installed SDK the lookup lands in site-packages and raises ConfigurationError at
import time, so the module can't be imported by tests at all.
